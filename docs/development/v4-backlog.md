...
| E10 | **Translations for the new namespace** | Open — unblocked by X1 | English only today. The plan: mine the dormant sections in `src/translations/editor-languages/` for anything reusable, translate, then delete them. X1 moved them off the eager path, so the ~+18,000 B gzip this was budgeted at is now downloaded by HACS and parsed only by browsers that open the editor — translate to whatever depth is worth doing, and a partial translation is safe because each key falls back to English on its own. **Measured, so the scope decision is a choice rather than an open question.** Sampling German, the language with the fullest dormant file: the new editor needs **306** string keys, `de.json` carries **239**, and they overlap as **90 exact key matches (29%)**, **99 that match only on the leaf name (32%)** — same last segment under a different group, so reusable but *not* safely automatic — and **117 with no counterpart at all (38%)**, which are the search UI, the exceptions widget and the per-view notes. So even a perfect mine leaves well over a third untranslated, in the 11 languages that have a file at all; the other 24 have nothing to mine. **This is safe to do incrementally**, because `translateEditorKey()` resolves per key — requested language, then English, then the raw name — so a partial file renders the rest in English rather than as key names, and `check:i18n` reports the gap as a warning. The judgement the maintainer owns is whether a form that is 29% German and 71% English reads better than one that is wholly English; that is a taste call about mixed-language UI, not a technical one, and it decides whether this is one mechanical pass or a community effort. |
