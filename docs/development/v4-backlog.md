...
| E13 | **`features/editor.md` still describes the editor that was replaced** | Open | Its *Editor Organization* list names seven panels — *Calendar Entities*, *Core Settings*, *Appearance & Layout*, *Date Display*, *Event Display* — which is the pre-rebuild taxonomy; the editor now has nine, named for what they configure. *Smart Validation* and the one-click *Configuration Upgrader* also describe behaviour of the old element: the new write path prunes the five dead keys silently on save rather than offering a button. Nothing fails, because `check:docs` reconciles options and links rather than prose. Wants one pass once the panel set has settled, together with a fresh screenshot. |
