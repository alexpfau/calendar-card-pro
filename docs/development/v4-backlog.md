...
| E7 | **UV index and low temperature are mutually exclusive** | Open | The card applies this rule in the day header; the editor does not express it. Being specified with the weather work below. |
...
| C2 | **Per-event weather row** | In progress | In list view, `show_conditions` gates the icon. In column view the weather now has its own row, so switching conditions off leaves a row holding only a temperature, breaking the leading icon edge that time, location and description share. Proposal: in column view the row and its icon are always shown once per-event weather is on, and `show_conditions` instead controls showing the condition **verbally**. Feasibility turns on whether HA's condition vocabulary is reachable as translated text from a custom card. Spec being written to `weather-column-view-spec.md`. |
