...
| E15  | **`strings.ts` capitalises option labels inconsistently** | Open — found by Stage 0, characterised here | Surfaced while Stage 0 built `checkCollapsedLabels`: `height_mode.option.maximum.label` says `Maximum height` where `card_max_height` and `column.max_height` say `Maximum Height`, so **all nine languages collapse the pair to one string** and a naive check would blame nine translation files for one inconsistency in the English table. Stage 0 excluded capitalisation-only pairs, which is right. **The pair is not the finding — the class is.** Measured over `EDITOR_STRINGS`: multi-word **field** labels are Title Case 144 of 161 (89%), but multi-word **option** labels are split **21 Title / 18 sentence** (54%) — `A Fixed Date`, `Relative To Today`, `Pulsing Dot`, `In The Day Header` against `Keep as one event`, `Maximum height`. So there is no option-label convention to be inconsistent with; the collision is the one place the absence becomes visible. Deciding one and applying it is cheap now and gets expensive the moment nine languages have translated all 39 — each change would invalidate a translated string in nine files. **Do it before Stage 1, or accept it permanently.** **The 54% is the load-bearing number and it is classifier-independent** — Stage 0 re-measured with a stricter rule (*every* non-initial word capitalised, rather than *any*) and got the same 21/39. The field-label figure is not: the same two rules give 144 or 138, because six strings differ, and most of those are prose that this classifier counts as a label. Re-measure the option split, not the field one. Note the sentence-case option labels are not obviously wrong: several read as sentences (`Keep as one event`), which is an argument for sentence case as the option convention rather than for Title Case everywhere. |
...
| Y7  | **Weather conditions follow Home Assistant's language, not the card's** | **Done** — maintainer ruled they must follow the card's             | Noticed while verifying the C6 weather row on a `language: 'en'` card, which rendered `Sonnig`, `Klare Nacht` and `Teilweise bewölkt`. `formatCondition` called `hass.formatEntityState()`, which resolves against the translations loaded for the **signed-in user's** profile language and takes no language parameter, so the card's `language` could never reach it. **Fixed without shipping a single condition string**, which was the property worth keeping: `frontend/get_translations` (`category: entity_component`, `integration: weather`) returns the same vocabulary Home Assistant would have used, in any language, so all 35 card languages work at once and nothing needs maintaining as HA adds conditions. Fetched once per language into `utils/weather-i18n.ts`, cached, in-flight guarded; the render path stays synchronous and reads the cache. **Skipped entirely when the card's language already equals the instance's** — the common case never leaves today's path. Three things had to be measured rather than assumed, all against 2026.8.1. **An unknown language code is not an error**: HA answers it with English, successfully, so a wrong code is indistinguishable from a working one by the shape of the reply and only a payload comparison across codes can detect it. On that evidence the card's `zh-cn`/`zh-tw` return English and must be mapped to HA's script subtags `zh-Hans`/`zh-Hant`, and regional codes are case-sensitive (`en-gb` ✗, `en-GB` ✓). The other 32 codes pass through unchanged. **Thai has no HA translation at all** and degrades to English — an HA coverage gap the card cannot close. First paint before the fetch resolves shows the instance's wording, never a blank row or a raw `partlycloudy`, and every failure returns exactly the pre-fix behaviour. A miss is diagnosable at both layers rather than only the raw-token one: a payload arriving short of the fifteen is reported when it lands, and a condition the fetched language lacks is reported once per language/condition — otherwise the fallback _succeeds_ in the instance's language and puts a German word inside an English card with nothing raised. Cost, measured on the shipped chunk: **+764 B gzip (+2,273 raw) on the card, 0 B on the editor — byte-identical**. Stated in [`features/weather.md`](https://calendar-card-pro.alexpfau.com/features/weather), which previously asserted the opposite.                                                                                                                                                                                                                                                                                            |
