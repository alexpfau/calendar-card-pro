...
**Settled at `80%`** *(maintainer, after seeing `75%` live)*, marked in `styles.ts` as the
...
rounded bar in a 20% tint, flush to both edges of the event, reads as a rule *between* rows
...
wrapped countdown, which C5 fixed by left-aligning it, was landing under the *clock icon*
...
| Y7 | **Weather conditions follow Home Assistant's language, not the card's** | Open — needs a ruling | Noticed while verifying the C6 weather row on a `language: 'en'` card, which rendered `Sonnig`, `Klare Nacht` and `Teilweise bewölkt`. `formatCondition` (`utils/weather.ts`) calls `hass.formatEntityState()`, which localises to the **HA instance** locale; the card's `language` option never reaches it. **There is a real case for leaving it**: a condition is entity *state*, HA owns its localisation, and `formatEntityState` is HA's own API — the card's `language` arguably governs the card's own strings (weekdays, *All day*, countdowns) rather than data it is displaying. **And a real case against**: a user who sets `language: en` on a German instance reasonably expects an English card, and gets one English sentence with a German word in it. Pre-existing since the per-event weather row landed, not introduced by the narrow-width fix. Whichever way it goes it should be *stated* in `docs/features/weather.md`, which currently says nothing about which language a condition appears in. |
...
| Y5 | **`event_icon_vertical_alignment` does nothing to the time row** | **Done** — fixed, and snapshot-safe | The shared `.time, .location, .description` rule sets `align-items: var(--calendar-card-event-icon-vertical-alignment)`, and `.time`'s own later rule — same specificity, so source order wins — hardcodes `align-items: center` straight over it. So the option works on the location and description rows and is inert on the time row, in **both** views. Worse, even without the override it could not have positioned the *time icon*, which is nested inside `.time-actual` rather than being a child of `.time`; `.time-actual` hardcodes `center` too. A user setting `top` or `bottom` gets two rows out of three. **Deliberately not fixed in C5**: the correction lands in the shared rule and would move list-view rendering for anyone who set the option, which C5 was forbidden to do. Needs its own change with its own snapshot review — and a decision on whether "event icon alignment" is meant to reach the icon inside `.time-actual` (a second declaration) or only the row's own children. **Fixed by honouring the variable on `.time-actual` rather than on `.time`.** That answers the open question the entry raised: the option means *align the icon against its own text*, and `.time-actual` is the container whose children are (icon, text) — the same shape `.location` and `.description` have. `.time` keeps `align-items: center`, because its children are the wrapper plus a countdown or progress bar, and restoring the variable there would tilt those while leaving the icon centred. **The feared list-view churn does not happen:** the option defaults to `middle`, which resolves to `center`, so a card that never set it is byte-identical and the DOM goldens did not move — only a user who set `top`/`bottom` changes, which is the repair. Three tests assert the property (containers whose children are icon+text) rather than a list of selectors; reintroducing the hardcode fails two of them. **Verified live at `?v=291` against the HACS production build as a control**, by reading computed styles rather than eyeballing pixels. With the option set to `top` — production: `.time-actual` `center` (the bug), `.location` `flex-start`; dev: `.time-actual` `flex-start`, `.location` `flex-start`. `.time` reads `center` in both, which is the intended difference. **Superseded in part by Y8**: the default is now `top`, so the resolved value is `flex-start` and the "defaults to `middle`, which resolves to `center`" argument above is history rather than current state. |
| Y8 | **Countdown breaks atomically; icon alignment defaults to `middle`** | **Done** — one markup change, one default change, both breaking-ish | Two reports the maintainer raised together after seeing the C6 weather fix live, and they are one problem seen from two sides. **The countdown.** `.time-countdown` was a sibling flex item of `.time-actual`, and a flex item is placed as a unit, so `7:00 - 23:59 · in 7 hours` could only ever break *between* the two — Chromium takes the whole countdown to a second flex line before it will look at the spaces inside either string. Fixed the same way C6 fixed the weather condition, and deliberately so: a new `countdownPlacement: 'text'` option on `renderEventContent` nests the time and the countdown inside one `.time-text` span, which is `.event-weather-text` by another name. **CSS alone could not do it**, and the note at `EventContentOptions.progressPlacement` was right about why — dissolving `.time`'s flex would have stacked `.time-actual` and the countdown vertically and taken `align-items` out of play. Moving the countdown *inside* `.time-actual` keeps the flex row and gets the inline formatting context, which is the thing C5 wanted and could not reach from the stylesheet. **The list view is untouched, markup included**: its countdown is not a continuation of the time (no separator, `space-between` parks it at the far right of a card-wide cell), so it keeps `'trailing'`, and the wrapper is emitted only when there is actually a countdown to fold — which is what keeps `column-dom.test.ts`'s strict byte-identity gate under default config. **The default.** With the time row now able to wrap, `event_icon_vertical_alignment: 'middle'` centres the clock against a two-line block, level with neither line — which is what prompted the maintainer to ask for `top`. Shipped for **both** views, not column-only, and that is a decision about the mechanism rather than taste: `COLUMN_DEFAULT_OVERRIDES` does not mean "a different default", it means the key **stops inheriting the top-level value entirely**, so `middle` written at the top level would be silently ignored in column view until repeated inside `column:`. Worth that price for `show_empty_days`, whose alternative is a grid that no longer corresponds to consecutive days; not worth it for an icon. **Breaking for anyone who liked `middle`** — documented as such on `layout-appearance.md` with the one-line restore. **The whole suite stayed green across the default change and no snapshot moved**, because the option resolves to a host custom property and is invisible to both DOM goldens — which is exactly the blind spot `AGENTS.md` warns about, and why `tests/icon-alignment.test.ts` now pins every value explicitly rather than leaving one covered only by being the default. |
| Y9 | **Weather condition escapes its column at larger font sizes** | **Done** — a one-word regression from Y6's own commit | Reported live: at a larger `weather.event.font_size` the condition ran sideways past its own column into the next one, with `Regnerisch` appearing cut off mid-word. **Three plausible causes, all wrong.** The track is not at fault (`.day-column` has `min-width: 0` and shrinks correctly); nothing was clipping the word (`.weather-condition` sets `overflow: hidden`, but at the default `max_lines: 0` its `display` resolves to `inline`, and **`overflow` does not apply to a non-replaced inline box** — the apparent clipping was the neighbouring column painting over text that had genuinely escaped); and it is not a flex-shrink problem. **The cause is one declaration**: `f6f59a0` changed `.weather-condition` from `overflow-wrap: break-word` to `normal`, and under `normal` a word wider than its container cannot break at all and simply overhangs. **Why it looked safe is the interesting part.** `break-word` genuinely *was* dangerous once — while the separator was in-flow content it could break after the middot and strand it alone on a line — but `0c40e7e`, one commit earlier, had already fixed that by making the `::before` absolutely positioned, which removes the hazard *structurally*: out of flow, the dot is not part of any character sequence a break can land inside. The two changes got read as one, and the second protected against nothing while costing the row its only defence against a long word. Restored to `break-word`, with the pairing now asserted as a pair — if anyone puts the dot back in flow, `stylesheet.test.ts` fails and `break-word` has to be reconsidered in the same edit. **The default path is what the test asserts**, because setting `max_lines` makes the element a `-webkit-box`, `overflow` starts applying and the symptom hides itself. **`.column-date-content .weather` does not share the bug** despite also carrying `overflow: hidden` — it is a grid item, so it is blockified and its `text-overflow: ellipsis` genuinely applies, which is the intended behaviour for a single-line header badge. |
