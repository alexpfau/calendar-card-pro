...
> Rule 2 domain mismatch §3.5 warns about. At HA's *calendar* keys German says
...
| location | HA `Standort`/`Speicherort`; editor `Ort` | **Ort** | *(see note)* — read at a **device** key; HA's calendar key also says `Ort`, so there is no disagreement |
