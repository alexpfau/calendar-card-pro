...
**No evidence in any source.** Genuinely undecided in two languages, and one of the
highest-risk terms precisely because nothing external will arbitrate it.

|             | de               | et        | it  | lt              | lv             | nb            | pl            | sk  | sv        |
| ----------- | ---------------- | --------- | --- | --------------- | -------------- | ------------- | ------------- | --- | --------- |
| **Decided** | Tagesüberschrift | Päevapäis | —   | Dienos antraštė | Dienas galvene | Dagoverskrift | Nagłówek dnia | —   | Dagrubrik |
...
**Estonian, Lithuanian and Latvian decided by the Baltic session (Stage 1), and by the same
route Polish took independently.** HA's `ui.panel.lovelace.editor.header-footer.header`
gives `Päis`, `Antraštė` and `Galvene` for _Header_, and each language's own compounding
rule settles the shape — Estonian compounds solid (`Päevapäis`), Lithuanian and Latvian take
...
**Seven of nine now agree on the method, which is worth more than the seven words.** Every
session that decided this reached for HA's `header`/`heading` key and then applied its own
compounding rule, and none of them consulted each other. The entry that opened "no evidence
in any source" was wrong about the evidence: there is none for _day header_, and there is a
settled one for _header_ in all nine. Only `it` and `sk` remain.
...
> **That warning immediately caught a Latvian defect in this session's own file, which is
> the argument for it.** `week_number_mode.option.simple.label` shipped as `Vienkāršs`
> (masculine) under a control this session had just labelled `Numerācija` (feminine), and
> was corrected to `Vienkārša` — the identical defect Polish found at the identical key,
> in a language the Polish session never touched. Two independent instances at one key
> makes it a property of the key rather than of either language: **`week_number_mode` is
> the picker whose control noun a session is most likely to name only after writing its
> options.** Italian and Slovak should check it first.
