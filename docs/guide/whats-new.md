...
- 🏷️ **All-Day Events, as a Pill**: [`allday_badge`](/features/event-content#the-all-day-badge) marks an all-day event with a rounded pill in its calendar's own color — wrapped around the event title the way Google Calendar and Apple Calendar draw it, or beside the clock in place of the words _All day_ — and `allday_badge_style` picks one of five treatments from a bare outline to a solid chip. `show_single_allday_time` and `show_multiday_allday_time` then drop the time row the pill has made redundant, and `show_location_allday` and `show_description_allday` drop the venue and the description, so an all-day event can read as just its pill
