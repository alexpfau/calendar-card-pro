...
- 🏷️ **All-Day Events, as a Pill**: [`allday_badge`](/features/event-content#the-all-day-badge) marks an all-day event with a rounded pill in its calendar's own colour — beside the clock in place of the words _All day_, or wrapped around the event title the way Google Calendar and Apple Calendar draw it — and `allday_badge_style` picks one of five treatments from a bare outline to a solid chip. `show_location_allday`, `show_description_allday` and `show_multiday_allday_time` strip the supporting rows so an all-day event can read as just its pill
