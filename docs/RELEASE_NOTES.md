...
`view: column` lays the days out side by side, one column each, instead of stacking them. It is the same agenda the list layout already shows, rotated: every event, label and color option you use today applies unchanged, and the date moves from the left of each row to the top of each column. The default, `list`, is what the card has always done (#377, #14, #263, #253)
