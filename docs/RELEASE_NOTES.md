...
**Days side by side in a new column layout, a visual editor rebuilt from scratch, and a card file 42% smaller to download on every dashboard load.** Calendar Card Pro has had one shape since it was released: a list of days, each stacked above the last. This release adds a second — and rebuilds the editor you configure it with, because a layout that carries its own defaults, its own per-view overrides and its own density rules needs an editor that can express them, and the hand-written one could not. Moving that editor into a file of its own is what pays for both: it is now fetched the first time you open it rather than downloaded by every dashboard, which leaves the card itself substantially smaller than it was before any of this was added. The column layout is entirely opt-in — a card that does not set `view` keeps the list it has always had — but three things do change for everyone, so they are listed first.
...
- **`first_day_of_week: system` Now Follows Home Assistant** - `system` is the default, and the editor labels it _Follow Home Assistant_, but it returned Monday for every user on earth regardless of profile or language. Upgrading therefore shifts week numbers, week separators and the `start_date: start_of_week` anchor for anyone whose Home Assistant profile or language implies a week that does not begin on Monday — nineteen of the 64 languages Home Assistant ships do not, seventeen starting on Sunday and Arabic and Persian on Saturday. Set `first_day_of_week: monday` to keep the old behavior. Two separate faults produced it: the branch never read the **First day of week** setting in the Home Assistant user profile at all, unlike its sibling `time_24h: system` which does read the matching profile setting, and its language fallback tested for `en-US`, `en-CA` or `es-US` in a string that has always been lowercased before it arrives and is in any case one of the card's own translation codes rather than a Home Assistant locale, so the branch could not match anything. The setting is now resolved the way Home Assistant resolves it — an explicit weekday in the profile wins, and only when that is left at _Auto_ does the language decide — and the card's own `language` option is no longer consulted, since it selects a translation and doubles as the fallback for the thirty-odd languages the card has no translation for, which makes it silent about regional convention. The replacement table is pinned against the platform's own locale data for all 64 languages, so it cannot drift; see [Week Numbers & Visual Separators](https://calendar-card-pro.alexpfau.com/features/layout-appearance#week-numbers-visual-separators)
- **Weather Badges Are Styled Through Custom Properties** - The weather icon and text carried their font size and color as inline `style` attributes, which no theme could override. Those attributes are gone, and the six `--calendar-card-weather-*` properties the card was already emitting are now read by the stylesheet instead, making them a real override surface. The event badge also gained an inner `.event-weather-text` wrapper so its parts wrap as running text rather than as three unbreakable blocks. Two things follow. Card-mod rules that targeted the inline-styled elements need updating — the properties are listed under [Weather Custom Properties](https://calendar-card-pro.alexpfau.com/features/theming#weather-custom-properties). And `weather.date.color` / `weather.event.color` no longer carry a default. They did in 3.x, but the default was unreachable: `setConfig` merges shallowly, so any card that actually shows weather supplies a `weather:` block, and that block replaced the whole default sub-tree. The rendered color came from a fallback in the renderer either way — primary for the day header, secondary for the per-event badge — and both fallbacks now live in the stylesheet with the same values, so the badge looks as it did

## 🎉 New Features

### 🗓️ Column View

`view: column` lays the days out side by side, one column each, instead of stacking them. It is the same agenda the list layout already shows, rotated: your events, labels and colors carry over as they are, and the date moves from the left of each row to the top of each column. Two options start from a different default in a column layout — see below. The default, `list`, is what the card has always done (#377, #14, #263, #253)

- **Responsive by Design** - A day column has a minimum readable width — `column.min_day_width`, `140` pixels — and a card too narrow to give every day that much room renders as a **list** instead. That means both layouts are live for the same card, which is why the editor annotates list-only options rather than hiding them: hiding a control would remove it for the layout that same card uses on a phone
- **Fewer Columns Before Falling Back** - `column.min_days_to_show` is the fewest columns the card may shrink to, letting a narrow card drop trailing days one at a time instead of abandoning the column layout outright. It defaults to `days_to_show`, which switches the behavior off — a card showing four of seven days looks exactly like a card configured for four days, so reducing columns is opt-in rather than something that happens to a config you already had. `column.min_days_fallback: cramp` holds the floor and lets the columns narrow past `min_day_width` instead of falling back at all
- **Per-View Values with `column:`** - A value tuned for a full-width row is often wrong in a column, so a nested `column:` block holds the values that apply only when the card renders as columns. Anything the block does not mention keeps its top-level value. It works in both directions, so an option switched off at the top level can be switched back on for columns. Only presentation options may appear there — anything deciding _which_ events are loaded has to hold one value in both layouts, because the card switches between them as the dashboard resizes and a per-view value would mean refetching on every crossing (see [Overriding Options in Column View](https://calendar-card-pro.alexpfau.com/features/column-view#overriding-options-in-column-view))
- **Two Options Start From a Different Default** - `show_empty_days` and `split_multiday_events` both default to `true` in column view and do **not** inherit the top-level value. A list of events reads perfectly well with the blank days left out; a row of day columns does not, because dropping the empty ones stops the columns corresponding to consecutive days. The same reasoning covers a multi-day event — a column _is_ a day, so an event spanning three of them belongs in all three (see [Options That Start From a Different Default](https://calendar-card-pro.alexpfau.com/features/column-view#options-that-start-from-a-different-default))
- **Separators Rotate into the Gutter** - Day, week and month separators keep their existing options and become full-height vertical rules between columns, with month taking precedence over week and week over day. A new column-only rule can also run under each day header — `column.day_header_separator_width` and `_color` — and defaults off, like every other separator in the card. `column.day_header_gap` owns the space between a header and its first event, so switching the rule on centers it inside the existing space rather than shifting the layout
- **A Header Per Column** - Week numbers render in each column's header rather than as a full-width row, with columns that do not start a week reserving the exact height without announcing anything to assistive technology. The today indicator becomes a leading item on the weekday row — an unambiguous marker at any column width — because the default `15% 50%` position is calibrated for the list view's narrow date cell and resolves _into_ the day number on a full-width column header
...
The editor is rebuilt from scratch on Home Assistant's own `<ha-form>`, where each panel is a schema fed to HA rather than a hand-assembled tree of components. That is the point rather than an implementation detail: a schema names a _selector_, never an element, so when Home Assistant renames an input component the card is not exposed to it. The old editor named a dozen HA elements and lost its text fields entirely when `ha-textfield` was removed in 2026.5; the new one names three.

- **Nine Panels Named for What They Configure** - Calendars, Card & Title, Time Range & Content, Events, Day Header, Layout, Separators, Weather, and Actions & Refresh — named for what they configure rather than for where the option happens to sit in YAML. Panels open one at a time, and options inside them appear only when they apply
- **Search** - A search box above the panels filters every panel down to what matches and drops the panels, and the groups inside them, that have nothing left — so a match is never hidden behind a collapsed heading. It matches what is on screen: the name of a setting, the sentence explaining it, and the choices a dropdown offers. It also matches the YAML option name, so a key copied out of the configuration reference leads straight to its control
- **Customized Only** - Hides everything still set to the value the card would use anyway, leaving exactly what this card changes. It reads values the way the card does, so a number written as `"3"` counts as untouched when `3` is the default (see [Search & Customized Only](https://calendar-card-pro.alexpfau.com/features/editor#search-customized-only))
- **Per-Calendar Settings** - Each configured calendar gets its own collapsible form, gathering all eleven per-calendar options — the label, three colors, four inheritable switches, the two filters and the compact limit — behind one header, plus a new selector for the shape of the label and a clipboard for copying settings between calendars. Four of those are genuinely three-state: an unset option means "follow the card", which no checkbox can express, and the editor this replaces rendered them as unchecked and wrote a literal `false` on first touch that nothing could take back
...
- **Label Type Read Back From the Value** - A calendar's label can be text, an emoji, an icon or an image, and most of the time the type is not stored at all — it is read back from the value, because the value is usually enough to decide how the card draws it. So a calendar configured in YAML opens with the right control already selected, without a `label_type` option anywhere in it, and choosing **An Icon** gives you Home Assistant's icon picker rather than a box you have to know `mdi:` to use. The type is written out only when the value alone would give the wrong answer — asking for text and then typing `mdi:home` — in which case `label_type` is stored beside `label` and wins over the value

### 🌍 Nine Fully Translated Editor Languages

The editor is now completely translated into **German, Estonian, Italian, Latvian, Lithuanian, Norwegian Bokmål, Polish, Slovak and Swedish** — 312 of 312 strings in each. The rebuild came with a new and considerably larger string table, so every one of these was re-translated against it rather than carried over; none was lost along the way. British English is generated from a decided termbase and carries only the strings where it differs from US English, and English itself now lives in code rather than in a translation file, so there is no second English table to disagree with it.

Translation resolves **per string**, requested language first and English second, so a partial file renders in that language for everything it covers and in English for the rest rather than looking broken. That makes a partial contribution genuinely useful, which is why the editor's strings now live in their own directory: they are the larger half of the card's translations, and they are loaded only when someone opens the editor, so a dashboard never downloads them at all. The calendar itself continues to speak 35 languages.
...
- **Runtime Log Level** - A released build prints errors only, so every warning the card raises is invisible to real users — including actionable ones such as an invalid `start_date` falling back to today. Someone filing a bug therefore could not produce the output that would explain it. Running `window.calendarCardProDebug = true` in the browser console turns the detail back on, and `window.calendarCardProLogLevel = 0..3` selects a level directly. Neither persists and neither needs a reinstall: the flag lives on `window`, and the card reads it on its next render, so reproduce the problem without reloading — a reload discards the flag (see [Reporting a Bug](https://calendar-card-pro.alexpfau.com/contributing#reporting-a-bug))

## 🐛 Bug Fixes

### Configuration

- **Options Removed in v3.0.0 Were Ignored in Silence** - Five options were deleted from the runtime in the v3.0.0 cleanup, and the visual editor still offers a one-click upgrade for them. Anyone who configures the card in YAML and never opens the editor got no signal at all: the value was dropped, the replacement option took its default, and nothing said so. Because the failure is silent and cosmetic, it reads as "the update changed my styling". The card now names each removed option it finds and its replacement on the console — and does so on production builds, which is the whole point, since a warning suppressed in a release would have looked correct in testing and reached nobody
- **Cached Events Ignored Configuration Edits** - The cache stored the _processed_ event list while its key covered only the fetch inputs, so every other option involved in processing was invisible to it — an edited per-calendar label, color or toggle was ignored until the entry expired. A second defect had a mechanism no wider key could have fixed: the cache round-trips through JSON, so a restored entity config could never be reference-equal to the live one, and per-calendar event limits silently degraded to a coarser bucket on every cache hit. The cache now holds the raw calendar payload and processes it on every read, so a hit and a refetch provably agree. Two calendars configured against the same entity also rendered with the last block's settings for both, which the same change fixes
- **Sizes Typed Into the Visual Editor Were Thrown Away** - A bare number is not a CSS length, so the card already rewrote `day_spacing: 10` into `10px` when it came from YAML. The editor renders fifteen of these options — every spacing, font size, icon size and separator width — as free-text fields, and a text field hands its value back as a string, so typing `10` stored `"10"` and the number-only guard passed it through untouched: the same silent failure, on the input path most people actually use, in the very file whose fix for it was already there. The consequence is worse than an ignored value, because a browser drops the entire declaration rather than the offending token — `padding: var(--calendar-card-event-spacing) 0 var(--calendar-card-event-spacing) 12px` lost its `12px` as well, and a typed `additional_card_spacing` flattened `calc(... + 16px) 16px` to nothing, leaving the card with no padding at all and looking markedly worse than if the option had never been set. Nothing caught it because the rewrite was only ever tested with numbers, the editor tests assert what the schema declares rather than what a typed value becomes, and no check joined the two; the affected options are now derived from the live editor schema, so any option that later becomes a text field is covered without anyone having to remember. Two column-only sizes — `column.day_header_gap` and `column.day_header_separator_width` — needed a second pass to reach: they are not in the table the rewrite reads its defaults from, so it could not see they were lengths and returned them untouched. Both default tables now share one coercion, and both are intersected with the live schema, so a column-only size added later is covered on the same terms as every other
- **A Separator Set to Zero Still Took Up Its Space** - Whether a separator is drawn at all is decided by a zero test that recognised only a literal `0` followed by a unit, and that is not what reaches it. The three separator widths are free-text fields in the visual editor, so a typed `0.0` or `-0` arrives as `0.0px` or `-0px` once the fix above supplies the missing unit — zero lengths the test called non-zero. Because a week or month rule's margins do not scale with its border, the card then emitted an invisible separator carrying a full `day_spacing` margin above and below it, so asking for no separator produced the gap without the line. Hand-written CSS hit it too, since `0.0px` and `00px` are perfectly valid lengths. Nothing caught it because the option still _worked_ in the sense that mattered to a test — the border really was zero — and the only visible symptom was spacing no assertion was watching. The numeric part is now parsed rather than matched against a single character, so every spelling of zero counts as one
- **A Negative Day Spacing Made Column View Ignore Its Own Width Floor** - `column-gap` accepts no negative value, so a browser discards the declaration and renders no gutter at all — but the width arithmetic that decides whether column view can render still _subtracted_ it, reserving space the layout was not saving. Three days with a 140px minimum width and a `-100px` gutter thresholded at 252px instead of 472px, so a 280px card drew three 83px columns against the 140px floor it had been told to honour, and nothing downstream re-imposed that floor because the grid tracks are `minmax(0, 1fr)` by design. The same value reached the separators as `calc(-0.5 * (gap + width))`, which turns a negative gap into a _positive_ margin that is perfectly valid and therefore survives, sliding every separator into the middle of a column. The number was guarded on one side and not the other: `min_day_width`, the other operand of the identical expression, has always rejected values at or below zero. A negative gutter is now replaced by the shipped default everywhere it is read, so what renders and what the card measured to decide to render it can no longer disagree; a gutter of exactly zero remains a legitimate request and is kept
- **Editing a Calendar's Own Settings Appeared To Do Nothing** - Per-calendar options — a label, a color, an icon, a per-entity event limit, an allowlist or blocklist — are not applied when the card draws. They are stamped onto each event while the calendar payload is processed, and every reader prefers that stamp over the live configuration, so changing one only takes effect when the payload is processed again. The card decided whether to do that by comparing the entity _identifiers_ alone, which is exactly right for deciding whether to call Home Assistant again and exactly wrong for deciding whether to reprocess: editing any of the twelve other per-calendar options left the identifiers untouched, so nothing happened. In the visual editor the change appeared to be ignored, and stayed that way until the next scheduled refresh — up to thirty minutes on the default interval — or until the dashboard was reloaded. A filter was the sharpest version: an event the user had just excluded stayed on screen. Nothing caught it because the two tests around this both asserted the fetch decision, which was never wrong, and no test asked what a per-calendar edit was supposed to produce. The card now separates the two questions: an identifier or window change still refetches, while any other change to a calendar block reprocesses the payload already in the cache, with no additional API call. The comparison serializes the whole calendar block rather than naming the fields it knows about, so the next per-calendar option added is covered without anyone having to remember — a list of field names is precisely the kind that drifts
- **A Column Override Could Not Turn Multi-Day Splitting Back Off** - Splitting a multi-day event into one entry per day ran twice: unconditionally as events were fetched, against the raw card config, and again as they were grouped for display, against the view-resolved one. Grouping can only ever _add_ segments, never merge them back, so the earlier pass silently won every disagreement — and `column: { split_multiday_events: false }`, the documented escape hatch for keeping a single entry with an "until" date in column view, was defeated for anyone who had the top-level option switched on for list view. The resolved value really was `false`; the event had simply been split before anything thought to ask. Nothing caught it because the column test covering that escape hatch sets no top-level `true` and hands fixtures straight to the renderer, so the two passes never had anything to disagree about. Splitting now happens only where the effective view is known, which also stops the card building segments it will never draw: a two-week event in a five-day window was stored as five segments where one event now serves, and both render the same five days. Removing the earlier pass moved one responsibility with it — those segments used to be trimmed by the window filter that runs just after it, so that bound now travels with them, and days beyond the requested window can no longer push real days out of the card
- **An Event Ending Exactly at Midnight Gained a Phantom Extra Day** - The multi-day splitter decided whether an event crossed into the next day by comparing its end against the last millisecond of the day it started on. Midnight is one millisecond past that, so an event running 23:00 to 00:00 — a perfectly ordinary way for a calendar to express "until the end of the evening" — was classified as multi-day, and the final segment that always follows a split produced a second entry whose start and end were the same instant. The event appeared twice: once where it belonged, and once as a zero-length entry at the top of the following day. Column view is affected by default, since it turns splitting on unless a `column: { split_multiday_events: false }` override says otherwise; the list view needed `split_multiday_events: true`. Nothing caught it because a zero-length segment renders as an ordinary-looking entry rather than as anything visibly malformed, and every existing test placed its boundaries a comfortable distance from midnight, where the off-by-one-millisecond comparison and a correct one agree. The next day is now entered only when the event genuinely extends past the first instant of it, and the guard is fixed where the split is decided rather than where the last segment is emitted, so an event that really does run past midnight keeps its true end time. The regression runs in UTC and under two real daylight-saving zones, with the event deliberately starting on a 23-hour and on a 25-hour day, because a fix that assumed a day is always 24 hours long would pass under UTC and still be wrong twice a year
- **Changing Which Day the Week Starts On Left the Previous Week on Screen** - `first_day_of_week` moves the fetch window whenever `start_date` is week-relative, and three separate places that decide what the card is currently showing were built without it. The event cache keyed the raw setting, and the raw setting is not an identity: `system` is one string that resolves to a different weekday per Home Assistant profile, so a Sunday window and a Monday window shared a single cache entry and whichever was fetched first was served to both. The memoized count behind `hide_when_empty` compared events, config, view and language — all four of which stay identical across a profile change — so a card could stay visible on a window that no longer held anything, or stay hidden on one that did. And the card's query identity omitted the setting while the change detector counted it, so the two disagreed: a weekday change asked for a refetch, and if that refetch came back empty the card concluded the events already on screen still belonged to the query being asked and kept the old week's events indefinitely. Nothing caught it because the test written when this option was first classified as fetch-affecting passed the literal strings `sunday` and `monday` and asserted the resulting keys differed — two different strings trivially produce two different keys, so it could never have seen that one string produces two windows. The cache key now takes the resolved weekday, and its parameter is typed as a number so the raw value is rejected at the call site rather than silently accepted again; the count memo carries the resolved weekday too, and the query identity carries the configured one. Resolving it surfaced a trap the old shape hid — Sunday resolves to `0`, and the key was appended behind a truthiness test that would have collided Sunday with "no weekday at all" — so that test is now an explicit comparison. Only week-relative start dates are affected: an absolute `start_date` resolves to the same window whatever the week begins on, and is asserted to
- **A Slow Reply From the Previous Calendar Could Replace the New One** - Fetching events is asynchronous, and the card decided what to do with a reply only once it had arrived, reading its own identity at that moment rather than the one the request was issued under. Every keystroke in the visual editor calls `setConfig`, which regenerates that identity and starts a fresh request, so two requests against two different calendars are routinely in flight at once — and their latencies are unrelated, so the older one can settle last. When it did, it committed the previous calendar's events _and_ stamped them with the current identity, which is worse than a stale render: the check that exists to notice events no longer matching the query then reported that they matched, so nothing downstream would correct it and the wrong calendar stayed on screen until the next scheduled refresh, up to thirty minutes on the default interval. Nothing caught it because every existing test resolves its requests in the order they were made, which is the one ordering where a guard and no guard behave identically. Each request now takes a monotonic ticket before it can await anything, and a reply whose ticket has been superseded discards itself rather than committing — including a failed one, which could previously mark the current query as broken on behalf of a query nobody was waiting on any more
- **One Card Load Asked Home Assistant for the Same Events Three Times** - Three independent paths start the first load — `setConfig()`, `connectedCallback()`, and the arm of `updated()` that fires when `hass` first arrives — and Home Assistant hands over both the configuration and `hass` before it attaches the card, so all three ran, and all three ran before any of them had written the cache. Each therefore found nothing cached and issued its own request: one configured calendar, three round-trips, multiplied by every calendar card on the dashboard and repeated on every page load. The editor's two live suggestion previews had the same shape from the other direction — they are deliberately built to share one cache entry so that showing both is affordable, but a shared entry only helps once it exists, and mounted together neither could see the other's request. Requests for the same calendars and window are now deduplicated at the point of the API call itself, so simultaneous callers wait on one round-trip; only the raw payload is shared, and each caller still applies its own display options to it. A fourth request came from the safety net: when Home Assistant attaches the card before it hands over its state, the card arms a retry a second and a half later, and that retry was only ever cancelled if the card was still waiting — never when the state arrived and the real load succeeded. It then ran anyway, deliberately ignoring the cache, well after the card had finished drawing. It is now disarmed the moment a load starts with usable state
- **One Malformed Event Could Empty the Whole Card** - Grouping, sorting and duplicate filtering all read an event's start and end without checking they are there, because Home Assistant's calendar API is supposed to supply both. The payload is actually produced by whichever integration backs the entity — CalDAV, ICS, Google or any third-party one — so an incomplete event does arrive in practice, and it did not cost that event its row: it threw, and the card rendered nothing at all, taking every other event and every other calendar down with it. Under the default configuration the bad event happened to fall out of the time-window filter before anything looked at it, which is why this only ever showed up for people who had turned `filter_duplicates` on, and why no test saw it. Events missing a start or an end are now dropped at both entry points, with a warning naming how many, and everything beside them renders
- **An Unfinished Weather Block Stopped the Visual Editor From Opening** - `weather:` is the only option with nested groups, and both of them — `date:` and `event:` — are blocks a user starts by typing the key on its own line. YAML reads an unfinished one as `null` rather than as a missing key, and the card has never minded: every read of a nested weather option is optionally chained, so the group falls back to its defaults and the card renders. The editor serializes that same configuration the moment it is handed one, and it walked each group without checking there was a group there to walk, so `date:` with nothing after it threw before the first field was drawn. That is the worst possible place for it, because the visual editor is what someone reaches for _precisely_ when their YAML is wrong — the card kept working and the tool for fixing it was the only thing that refused to open. A mistyped group was the quieter half of the same gap: a scalar does not throw, it enumerates, so `date: yes` was written back into the stored configuration as `{0: 'y', 1: 'e', 2: 's'}`. Nothing caught either because the editor's own tests build their configurations from the defaults, where every group is a real object. A group that is not one is now treated as absent, which is what the card already does with it
- **A Start Date That Never Existed Silently Became a Different Day** - A fixed `start_date` is checked part by part — a month of 1 to 12, a day of 1 to 31 — and then the assembled date is tested for validity. That test can never fail. JavaScript does not reject February 30; it rolls it forward into March, so the check passed, the warning promising a fallback to today never printed, and the card quietly showed a window starting three days after the one that was asked for. Every impossible day that clears a 31-day bound behaves this way — the 31st of April, June, September and November, and the 29th to 31st of February in a common year — and the further out of range the date, the further the window slides. Nothing caught it because the guard reads as though it handles this, and a card rendering a plausible window on a date nobody typed gives no sign anything went wrong. The parts are now read back off the assembled date, which is the only way to tell a date the runtime moved from one it accepted, so an impossible date falls back to today and says so
- **The Calendar Picker Overwrote a Repeated Calendar's Own Settings** - Listing one calendar twice is supported and useful — two blocks against the same entity can carry different labels, colors or filters, and the card is built to give each its own copy. The visual editor's calendar picker hands back a flat list of entity identifiers and has to pair each one with the block it came from, which it did through a lookup keyed by identifier. A key holds one value, so a repeated identifier kept only the last block, and every earlier duplicate was rewritten with the last one's settings the moment the picker was touched — including simply adding an unrelated third calendar, which is an action nobody would expect to alter the two already configured. The pairing now consumes each block in order rather than by lookup, so the first duplicate keeps the first block's settings and the second keeps the second's. Nothing caught it because the editor's tests configure distinct calendars, which is the one case where a lookup and an ordered pairing behave identically
- **An Unrelated Edit Dropped a Column Card Back to the List View** - Column view holds its answer inside a hysteresis band, so a card sitting near a boundary width does not flip between layouts as the dashboard resizes. That band only works if the card remembers what it is currently showing, and `setConfig()` — which runs on every keystroke in the visual editor and on every YAML save — re-fit the card from scratch, telling the resolver there was no previous layout. Inside the band the honest answer to "what fits here from nothing?" is narrower than the answer to "can what is already drawn stay?", so a full-width card showing seven columns fell out of column view entirely on an edit that had nothing to do with layout — renaming the card was enough — and stayed in list view until the next resize moved it out of the band. Nothing caught it because the resolver itself was never wrong, so every test that called it directly passed; only driving a real card through a measurement and then an edit can see it. The measured layout is now carried into the re-fit, and only once a measurement has actually happened, so the optimistic answer the card gives before it knows its own width still cannot seed the band
- **A Corrupted Cache Entry Was Rendered Instead of Refetched** - A cached entry was read back from browser storage and used on the strength of it being valid JSON. Anything that parsed was treated as events, and a text value was the dangerous shape: iterating a string yields its characters, so the card built a row per character and still counted the read as a hit, which suppressed the refetch that would have replaced it — the damage repaired itself only when the entry expired. The louder shapes failed loudly instead, taking the whole card down rather than one entry. Neither is reachable from anything the card writes today; both are reachable from an entry written by an older version, by another tab, or by any of the storage-syncing add-ons people run. A parsed entry is now checked for the shape the card assumes before it is trusted, and one that fails is evicted so the ordinary miss path refetches it
- **The Visual Editor Refused a Documented Event Limit** - `compact_events_to_show: 0` is a supported value, and a deliberate one: it is how a card is told to collapse to no events at all while staying expandable. The runtime accepts it, the documentation describes it, and v3 shipped a bug fix devoted to making that exact value expand and collapse correctly. The rebuilt editor renders the option as a number field, and a number field carries a floor — both the card-wide field and the per-calendar override were given a floor of one, so the single value the option exists to express could not be entered in the editor at all, and a card already configured with it could not be edited without losing it. The wrong floor looked right because `compact_days_to_show` sits in the same row and genuinely cannot be zero. Both floors now match what the card accepts, and every numeric floor in the editor is checked against the runtime that normalizes it rather than declared independently, so a field that will not accept a value the card supports is caught rather than shipped

### Appearance

- **Titles Ellipsized When Nothing Was Truncated** - A trailing `...` could appear on a title that fit
- **Metadata Rows Clipped Words Mid-Glyph** - Time, location and description rows cut text through a character rather than breaking at a word boundary
...
- **The Weather Badge's Weight Leaked Between Placements** - The day-header badge's semi-bold weight escaped into the event row variant, and the UV index kept a margin from the title row that doubled up with the separator, leaving it the only semi-bold text in the event block
- **The Event Weather Row Ran Together** - `.event-weather` is a flex container, so it discarded every space the template contained and the row rendered as `30°UV4Sunny`. Separators are now drawn from which pieces are present rather than enumerated per combination, so they never fire after the icon
- **Published Card-Mod Recipes Did Nothing in Column View** - Every card-mod example in the documentation selected `.day-table`, a class only the list layout emits, so each recipe silently stopped matching the moment the card was switched to column view — no error and no warning, just styling that quietly disappeared. The recipes now name both day containers, and `weekend` has been added to list view's `.day-table` so the two layouts expose the same four state classes — `today`, `tomorrow`, `future-day` and `weekend` — on the same element, letting one selector list cover both. `.date-column.weekend` is unchanged and still drives `weekend_day_color` and `weekend_weekday_color`

### Countdown

- **Split Multi-Day Events Showed Three Different Countdowns** - Splitting a multi-day event rewrites its middle days as all-day segments while leaving a real start time on the first day, so the all-day countdown fix applied calendar-day counting to only the middle rows — one event rendered `3 / 5 / 6 / 6` down the card where the days are `4 / 5 / 6 / 7` apart. Every segment the splitter produces is now counted in whole calendar days, while rows falling on today keep wall-clock precision so "in 2 hours" still works (#344)

### Week Numbers

- **Week Numbers Were Wrong for One Date in Seven Outside UTC** - Both numbering methods built their two dates from local components and then measured the gap between them in milliseconds, so a daylight-saving transition falling inside that gap left the elapsed time an hour short of — or an hour past — a whole number of days, and the rounding that followed tipped the result by a full week. The two methods round in opposite directions, which makes the defect a mirror image: `iso` was wrong only in the southern hemisphere and `simple` only in the northern, each on roughly one date in seven for as long as its zone observed DST. Sydney showed ISO week 15 on a date in week 14; Berlin and New York showed week 14 on a date in week 15. Nothing caught it because the test suite pins the zone to UTC — deliberately, so that dates render identically everywhere — and UTC is the one zone with no transitions and therefore the one zone where both functions were already correct. The arithmetic is now done in UTC internally, where a day is 24 hours by definition, which removes the failure rather than compensating for it, and the suite now runs these functions under a northern and a southern zone as well, since neither alone can see both halves of the bug. Verified against every date from 2015 to 2035 across seven zones

### Empty Days

- **The Last Day of the Calendar Could Lose Its "No Events" Placeholder** - The same defect as the week numbers above, in a second place. The empty-day filler measured how wide its window was by subtracting two locally built midnights in milliseconds, so a daylight-saving transition inside that window left the elapsed time an hour short of a whole number of days and the floor that followed dropped it to one day fewer. The loop then stopped a day early and the final day of the window was never filled, so a day with nothing scheduled vanished from the card instead of showing its placeholder. It needed three things at once — `show_empty_days` enabled, a window spanning the March or October transition, and no events on the last day of it — which is why it went unreported for so long. Unlike the week numbers this half was not hemisphere-specific, because every zone that observes DST springs forward and only the month differs. Both ends of the measurement are now reduced to their calendar date and differenced in UTC, and the same two-zone test run covers it

### Time Format

- **A 24-Hour Locale Could Still Get a 12-Hour Clock** - `time_24h` defaults to `system` and Home Assistant's own time format setting defaults to `language`, so for anyone who has changed neither the clock is chosen from `hass.locale.language` alone — and that choice was made by a hardcoded list of twenty-four "likely 24-hour" languages. It disagreed with the platform's locale data for 33 of the 64 languages Home Assistant ships. Three faults compounded: the region was discarded before the lookup, so `en-GB` was tested as `en` and took the American answer while `es-419` was tested as `es` and took the European one; the list named the macrolanguage `no`, which never matches the `nb` and `nn` tags Home Assistant actually sends, so both Norwegians missed; and Greek and Korean were listed wrongly outright. Ukrainian, Hebrew, Thai, Vietnamese, Icelandic, Catalan, Estonian, Latvian, Lithuanian, Indonesian, Persian, Georgian, Armenian and a dozen more read `1:00 PM` where their language writes `13:00`. The `system` branch carried a second, independent fault: it decided by searching the formatted string for `AM` or `PM`, which recognises only an unpunctuated Latin day period, so Greek `1 μ.μ.`, Traditional Chinese `下午1時`, Arabic `1 م`, Albanian `1 m.d.` and Latin-American Spanish `1 p.m.` were all read as 24-hour. Nothing caught either fault because the answer is correct for whichever language the person looking happens to use. Both are now a single query to the platform's own locale data, which agrees with it for all 64
...
- **The Editor Showed Five Weather Toggles Off While the Card Drew Them On** - Every nested weather option is read as "on unless explicitly false", so a `weather:` block that names an entity and leaves the rest out renders conditions and the high temperature in the date column, and conditions, the temperature and the daily-forecast fallback in the event row. The editor bound that block exactly as written, and a checkbox given nothing to bind draws unchecked — so all five sat off in the editor while the card was visibly drawing them, and the other ten defaulted weather controls read as unset beside them. The one panel whose job is to show you what the card is doing was the one panel contradicting it, and only for hand-written YAML, which is why it survived a rebuilt editor: a configuration saved from the editor writes the block whole and binds correctly, and a configuration with no weather at all inherits the full default. The weather block is now merged with its defaults for display exactly as the column block already was, and stripped back down on write, so the form states the truth without expanding anyone's YAML
...
- **Changing the Weather Entity Left the Old Entity's Forecast on Screen** - Switching to a different weather entity tore down the old subscription but never cleared the forecast it had already delivered, so the card kept drawing the previous entity's conditions and temperatures under the new configuration until the replacement subscription happened to emit. Where the new entity does not offer that forecast type at all — an entity with no hourly forecast, for instance — there was no later emission to correct it, and the old data simply stayed. The forecasts are now cleared when the entity changes, and only when it changes, so moving the badge or restyling it no longer blanks and redraws the weather for no reason
- **A Second Card Showed Untranslated Weather** - Weather conditions are translated by fetching Home Assistant's own translation resources once per language, and two cards asking at the same moment correctly make a single request. The card that arrived second, however, had its redraw callback discarded rather than queued: the translations were fetched and cached for everyone, but only the first card was told they had arrived. The second went on showing the untranslated English condition until some unrelated update happened to redraw it. Every card waiting on the same request is now notified
