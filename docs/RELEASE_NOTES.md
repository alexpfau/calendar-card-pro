...
- **The Card File Is 41% Smaller to Download** - The visual editor and its translations are the larger half of the bundle, and most dashboard loads never open it. Splitting the editor into a file the card fetches on demand takes it, and every string it needs, off the path every dashboard pays for. Measured on the production build, comparing v3.5.0 against v4.0.0: the file every user downloads falls from **96,870 to 57,055 bytes** over the wire, which is 41% — the same 41% at `gzip -9`, where it is 96,651 to 56,908. Uncompressed on disk the reduction is larger, 347,969 to 188,158 raw, or 46%. The editor's own 82,802 bytes are paid only by someone who opens it
