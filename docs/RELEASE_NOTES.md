...
- **A rule closing the time body** - The hour rules stopped an hour short of the bottom edge and nothing drew the band's own end, so the body trailed off into the card's padding — worst under a block carrying the "continues past the window" marking, which had nothing to continue past. The band's end is now swept like any other boundary: it is ruled where the cadence would have ruled it, and labeled where it falls on a whole hour. `end_time: 21:00` gets a line and a `21`; `21:30` on hourly rules gets neither; `21:30` at `slot_minutes: 30` gets the line without a label
