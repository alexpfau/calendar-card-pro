...
- **Label Type Read Back From the Value** - A calendar's label can be text, an emoji, an icon or an image, and the type is not stored anywhere — it is read back from the value, because the value is what decides how the card draws it. So a calendar configured in YAML opens with the right control already selected, and choosing **An Icon** gives you Home Assistant's icon picker rather than a box you have to know `mdi:` to use
