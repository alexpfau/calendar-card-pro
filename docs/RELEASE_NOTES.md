...
- **"Follow Home Assistant" Never Followed Home Assistant** - `first_day_of_week` defaults to `system`, which the editor labels _Follow Home Assistant_, and it did neither of the two things that name implies. It never read the **First day of week** setting in the Home Assistant user profile at all, unlike its sibling `time_24h: system` which does read the matching profile setting. And its language fallback tested for `en-US`, `en-CA` or `es-US` in a string that has always been lowercased before it arrives and is in any case one of the card's own translation codes, not a Home Assistant locale — so the branch could not match anything, and every user on earth got Monday regardless of profile or language. Nineteen of the 64 languages Home Assistant ships do not start their week on Monday: seventeen start on Sunday and Arabic and Persian start on Saturday. The setting is now resolved the way Home Assistant resolves it — an explicit weekday in the profile wins, and only when that is left at _Auto_ does the language decide — and the card's own `language` option is no longer consulted at all, since it selects a translation and doubles as the fallback for the thirty-odd languages the card has no translation for, which makes it silent about regional convention. Because week start also anchors `start_date: start_of_week`, upgrading will shift week numbers and that anchor for anyone whose profile or language implies a non-Monday week; set `first_day_of_week: 'monday'` to keep the old behavior. The new table is pinned against the platform's own locale data for all 64 languages, so it cannot drift
