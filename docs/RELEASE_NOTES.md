...
- **Event Titles Were Spaced on Home Assistant's Line Height, Not the Card's** - An event title is an inline span, so each of its line boxes is the taller of its own inline box and the strut of the block containing it. That block declared neither a font size nor a line height, so its strut came from Home Assistant's base typography — 22.4px — against the title's own `14px × 1.2 = 16.8px`, and won at every size the card supports. Two things followed. A wrapped title was spaced on Home Assistant's leading rather than the card's, and lowering `event_font_size` made that look worse instead of better, because the pitch stayed where it was while the glyphs shrank away from it — opening a gap wider than the lines it separated, which is why it was most visible in column view where titles wrap routinely. And because the strut applied to single-line titles too, `event_font_size` barely changed an event row's height at all: measured on a six-day card, taking it from `14px` to `11px` changed the rendered card height by **zero pixels**. The block now carries the title's own font size and line height, so the same change moves it by the 3.6px per title the option implies. Event rows are correspondingly a little tighter than in 3.x — about 5.6px per event at the default `event_font_size`
