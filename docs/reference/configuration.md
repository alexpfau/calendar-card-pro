...
| `title_max_lines`               | number            | `0`                                                | Maximum number of lines to show for event titles, including calendar labels on the first line (0 = unlimited). Truncated text shows `...`                                                                                                                                                                                                                             |
| `scroll_long_titles`            | boolean           | `false`                                            | When on, an event title too wide for its space is kept to one line and scrolls horizontally instead of wrapping. Only titles that actually overflow move; scrolling forces a single line and so overrides `title_max_lines`, and it never animates under `prefers-reduced-motion: reduce`. See [Scrolling Long Titles](/features/event-content#scrolling-long-titles) |
