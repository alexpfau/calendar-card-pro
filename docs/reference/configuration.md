...
| `day_spacing`             | string | `10px`                 | Space between days — vertical in list view, the gap between columns in column and grid view (replaces `row_spacing`). Grid view defaults this to `2px` inside `time_grid:`, so a block sits in the grid rather than floating inside it                 |
