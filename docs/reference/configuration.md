...
| `show_empty_days`              | boolean | `false`                       | Whether to show days with no events (with "No events" message). Column view defaults this to `true` and does not inherit the top-level value — see [Options That Start From a Different Default](/features/core-settings#options-that-start-from-a-different-default)                                                                                                                                                                                |
