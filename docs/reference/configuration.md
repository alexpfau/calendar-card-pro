...
| ------------------------------ | ------- | ----------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `config_version`               | number  | -                             | Editor-maintained configuration format marker. Current editor format: `5`. New cards and migrated editor configs write it automatically; YAML-only legacy cards may omit it and keep their existing runtime behavior                                                                                                                                                                                                                                             |
...
| `empty_day_text`               | string  | _translated default_          | Custom text shown on any day the card renders as empty, whether a gap between event days, an entire empty range, or the single row shown on the range's reference date when the card has nothing at all. Kept verbatim — never translated, trimmed or migrated. Omits the ✓ prefix when set; clear it to restore the date-neutral translated default                                                                                                             |
| `filter_duplicates`            | boolean | `false`                       | Hide eligible copies whose original title, start, end and location all match; the first eligible entry in `entities` supplies the colors, while shared labels name the eligible contributing calendars. Calendar date and expiry filters apply before choosing the winner                                                                                                                                                                                        |
...
| `time_grid → min_day_width`              | number  | `100`                                                           | Narrowest a day column may become, in pixels, before the grid sheds a column. Three days fit at 383px, or 399px entering from the list fallback                                                       |
...
| `time_grid → axis_width`                 | string  | `max-content`                                                   | Width of the hour-label gutter. The default sizes to the widest visible gutter label with fixed padding. `max-content` or a pixel length only; anything else folds back to the default with a warning |
...
by hand. When the editor first adopts an older card, it moves these unambiguous options
into `list:` automatically.
:::
...
| `day_spacing`             | string | `10px`                 | Space between days — vertical in list view, the gap between columns in column and grid view (replaces `row_spacing`). Grid view defaults this to `1px` inside `time_grid:`, so a block sits in the grid rather than floating inside it and the day rule exactly fills the gutter |
...
| `day_separator_width`          | string  | `0px`                         | Width of the vertical rule between days. Grid view defaults this to `1px` inside `time_grid:`; it draws no horizontal rule in any view                                    |
| `day_separator_color`          | string  | `var(--secondary-text-color)` | Color of separator line between days. Grid view defaults this to half-strength `var(--divider-color)` inside `time_grid:`, matching the gray its horizontal rules ship in |
...
| `past_event_opacity`            | number            | `60`                                               | Finished event content opacity, from 0 to 100 percent; fractions allowed. Also dims list and column empty-day notices once their own local date has ended, whichever way they were shown. `100` removes dimming; `0` makes contents transparent without hiding events. Backgrounds and accent stripes are unchanged. Shared or overridden in `list:`, `column:`, or `time_grid:` |
...
| `empty_day_color`               | string            | `--primary-text-color`                             | Color for the empty-day notice text. Unaffected by past-date dimming, which changes only opacity                                                                                                                                                                                                                                                                                 |
| `event_font_size`               | string            | `14px`                                             | Event title font size; Grid defaults to `12px` inside `time_grid:`. Short timed blocks try smaller vertical insets before scaling the whole label/title group in 1px font steps, protecting every text part's 10px floor or smaller authored size. See [Short Timed Titles](/features/grid-view#short-timed-titles)                                                              |
| `title_max_lines`               | number            | `0`                                                | Maximum title lines, including calendar labels (0 = unlimited). Truncated text shows `...`. Grid keeps the normal limit whenever the first line fits; its short-title fallback uses one static labeled line                                                                                                                                                                      |
| `scroll_long_titles`            | boolean           | `false`                                            | Keep overflowing titles on one line, overriding `title_max_lines`. Where supported, visible titles share forward starts and return individually to wait at their beginnings. Reduced motion and Grid's short-title fallback stay static. See [Scrolling Long Titles](/features/event-content#scrolling-long-titles)                                                              |
| `event_color`                   | string            | `--primary-text-color`                             | Event title font color. Accepts `accent` for each event’s own calendar color; grid view defaults this to `accent` inside `time_grid:`                                                                                                                                                                                                                                            |
...
| `time_color`                    | string            | `--secondary-text-color`                           | Event time font color. Accepts `accent` for each event’s own calendar color; grid view defaults this to `accent` inside `time_grid:`                                                                                                                                                                                                                                             |
...
| `location_color`                | string            | `--secondary-text-color`                           | Event location font color. Accepts `accent` for each event’s own calendar color; grid view defaults this to `accent` inside `time_grid:`                                                                                                                                                                                                                                         |
...
| `description_color`             | string            | `--secondary-text-color`                           | Event description font color. Accepts `accent` for each event’s own calendar color; grid view defaults this to `accent` inside `time_grid:`                                                                                                                                                                                                                                      |
