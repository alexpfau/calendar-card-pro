...
| `day_separator_width`          | string  | `0px`                         | Width of separator line between days. Grid view defaults this to `1px` inside `time_grid:`, the same width as its hour rules                                                                            |
| `day_separator_color`          | string  | `var(--secondary-text-color)` | Color of separator line between days. Grid view defaults this to half-strength `var(--divider-color)` inside `time_grid:`, the one value its vertical rules, hour rules and band frame are all drawn in |
