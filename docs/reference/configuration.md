...
| `past_event_opacity`            | number            | `60`                                               | Finished event content opacity, from 0 to 100 percent; fractions allowed. `100` removes dimming; `0` makes contents transparent without hiding events. Backgrounds and accent stripes are unchanged. Shared or overridden in `list:`, `column:`, or `time_grid:`                                                    |
...
| `scroll_long_titles`            | boolean           | `false`                                            | Keep overflowing titles on one scrolling line instead of wrapping, overriding `title_max_lines`. Does not animate under reduced motion. Grid's short-title fallback stays static; normal scrolling returns when height permits. See [Scrolling Long Titles](/features/event-content#scrolling-long-titles)          |
