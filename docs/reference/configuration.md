...
| `allday_badge`                  | boolean or string | `false`                                            | How the all-day label is drawn: `false` for plain text, or `outline`, `subtle`, `tinted` or `filled` for a rounded badge in the calendar accent color. Any end date that follows it stays as text     |
