...
| `filter_duplicates`            | boolean | `false`                       | Hide events whose title, start, end and location all match another event; the entry listed first in `entities` wins, which may be a second block of the same calendar                                                                                                                                                                                                                                                                               |
...
| `allday_badge`                  | string            | `off`                                              | Where the all-day pill is drawn: `off` for plain text, `time` to replace the all-day label beside the clock, or `title` to wrap the event title. A title pill is kept to one line and shortened with an ellipsis |
| `allday_badge_style`            | string            | `tinted`                                           | Which treatment draws the pill: `neutral`, `outline`, `subtle`, `tinted` or `filled`. Ignored while `allday_badge` is `off`                                                                                      |
