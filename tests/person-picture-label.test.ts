...
describe('recognising a person entity id', () => {
  it('matches a person', () => {
    for (const id of ['person.alex', 'person.a', 'person.mattis_2', 'person.jean_luc']) {
...
describe('reading a person id as a label shape', () => {
   ...
   * claimed it — drawing the characters `person.alex` in front of every event.
   */
   ...
   * one. `person` alone and `persons.alex` are words; `calendar.alex` is the shape a user
  ...
  it.each([
    ...
    ['a plural', 'persons.alex', 'text'],
    ['a calendar entity id', 'calendar.alex', 'text'],
...
describe('reading the picture Home Assistant holds', () => {
  ...
  /**
   * Checked against a live instance rather than assumed: `person.mattis` carries no `user_id`
  ...
  it('reads a person with no linked user', () => {
    ...
    expect(personPicture('person.mattis', hass)).toBe(PICTURE);
  });
});
...
describe('resolving one calendar’s label', () => {
   ...
   * draws. Returning `person.alex` would print the entity id, which is the defect this feature
   * exists to remove.
   */
  it.each([
...
describe('the editor’s image source round trip', () => {
   ...
   * no picture; `person.alex` sorts first and has one, so this passes either way unless the
