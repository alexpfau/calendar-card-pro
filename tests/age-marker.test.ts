...
describe('the YEAR marker grammar', () => {
  ...
  it.each([
    { description: 'YEAR=1976', year: 1976, why: 'the documented form' },
    { description: 'YEAR:1996', year: 1996, why: 'the form the reporter proposed' },
    { description: 'year=1976', year: 1976, why: 'lowercase' },
    { description: 'Year:1996', year: 1996, why: 'title case' },
    { description: 'Geboren YEAR=1996', year: 1996, why: 'after other text' },
    { description: 'Ralf\nYEAR=1996', year: 1996, why: 'on its own line' },
    { description: 'YEAR=1996\nRalf', year: 1996, why: 'on the first line' },
    { description: 'YEAR=1996.', year: 1996, why: 'followed by a full stop' },
    { description: 'YEAR=1996, Berlin', year: 1996, why: 'followed by a comma' },
    { description: 'Geboren\u00a0YEAR=1996', year: 1996, why: 'after a decoded &nbsp;' },
    { description: 'Wedding YEAR=2005', year: 2005, why: 'an anniversary, same grammar' },
  ...
  it.each([
    ...
    { description: 'YEAR = 1976', why: 'spaces around the separator are the prose form' },
    ...
    { description: 'BIRTHYEAR=1996', why: 'likewise, however reasonable it looks' },
    { description: 'YEAR=19766', why: 'more than four digits is not a year' },
    { description: 'YEAR=198', why: 'fewer than four digits is not a year either' },
    { description: 'YEAR=1996_x', why: 'a word character follows the digits' },
    { description: '(YEAR=1996)', why: 'a bracket is not whitespace' },
    { description: 'The year 1976 was a good one', why: 'no separator at all' },
    { description: 'Born in 1996', why: 'a bare year is not a marker' },
  ...
  it('takes the first marker when a description carries two', () => {
    expect(EventAge.readMarkerYear('YEAR=1996 and YEAR=1970')).toBe(1996);
  });
  ...
  it.each(['', 'Dinner with the team', 'Born in 1996'])('spends nothing on %s', (description) => {
...
describe('the count itself', () => {
  ...
  it('serves an anniversary with the same subtraction', () => {
    expect(EventAge.resolveAgeCount(2026, 2005)).toBe(21);
  });
...
describe('appending the count to a title', () => {
  it('appends in brackets', () => {
    expect(EventAge.appendAgeCount('Annas Geburtstag', 40)).toBe('Annas Geburtstag (40)');
  });
...
describe('stripping the marker out of a description', () => {
  ...
  it.each([
    { before: 'Geboren YEAR=1996 in Berlin', after: 'Geboren in Berlin' },
    { before: 'YEAR=1996 Geboren', after: 'Geboren' },
    { before: 'Geboren  YEAR=1996  in Berlin', after: 'Geboren in Berlin' },
    { before: 'YEAR=1996', after: '' },
  ...
  it('does not leave a run of spaces where the marker was', () => {
    expect(EventAge.stripAgeMarker('Geboren  YEAR=1996  in Berlin')).not.toMatch(/ {2}/);
  });
...
describe('an event on the card', () => {
  ...
  it('still shows no description, having read one', () => {
    expect(displayed(birthday('YEAR=1976')).description).toBe('');
  });
  ...
  it('keeps the marker out of the description it draws', () => {
    const event = displayed(birthday('Geboren YEAR=1976 in Berlin'), { show_description: true });
  ...
  it('draws no description at all when the marker was the whole of it', () => {
    ...
    expect(displayed(birthday('YEAR=1976'), { show_description: true }).description).toBe('');
  });
  ...
  it('leaves a timetable description alone', () => {
    ...
    expect(event.summary).toBe('Annas Geburtstag');
  ...
  it('does not touch the raw event it was handed', () => {
    ...
    displayed(raw);
    expect(raw.summary).toBe('Annas Geburtstag');
    expect(raw.description).toBe('YEAR=1976');
  });
  ...
  it('keeps birthdays in title order', () => {
    ...
    const events = EventUtils.groupEventsByDay(
      [
        ...
        birthday('YEAR=1976', 'Annas Geburtstag'),
