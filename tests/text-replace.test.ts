...
describe('ordering against the formatting the card already does', () => {
  ...
  it('rewrites a description after the age marker has been removed', () => {
    ...
    expect(
      drawn(
        allDay({ summary: 'Alex Geburtstag', description: 'Geboren YEAR=1986 in Berlin' }),
...
describe('the age count from #124', () => {
  const BIRTHDAY = { summary: 'Alex Geburtstag', description: 'YEAR=1986' };
  ...
  it('is appended to a title the pattern merely edited (#153 ex.1)', () => {
    ...
    expect(drawn(allDay(BIRTHDAY), { replace_pattern: ' Geburtstag' }).summary).toBe('Alex (40)');
  });
