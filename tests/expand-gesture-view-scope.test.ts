/**
 * The expand gesture is inert while the card renders column or grid.
 *
 * The whole compact family is list-only — `compact_events_to_show`, the per-entity form,
 * `compact_days_to_show`, `compact_events_complete_days`, and the `action: 'expand'`
 * gesture that drives them. Column/grid density is `min_days_to_show` /
 ...
 * Grid shares that gate, so it is pinned here too.
 *
 * That was not a harmless flag. The same flag gates the empty-day range branch, so with
 * `show_empty_days: false` and an empty range a collapsed card rendered one day and an
 * expanded one rendered seven — a visible layout change from a gesture the docs say does
 * nothing while rendering as column/grid.
 *
 * Gating the gesture alone is not enough: `setConfig` keeps `isExpanded`, so expanding
 * on list then switching to column/grid left the flag true and the empty-day branch still
 * widened. That leftover path is pinned in `leftover-expand-view-switch.test.ts`.
 *
...
describe('the expand gesture outside list view', () => {
  beforeEach(() => {
  ...
  it('does NOT expand in GRID view, even with a compact limit set', () => {
    const card = mount(config({ view: 'grid' }));
  ...
  it('list view with no compact limit at all does not expand', () => {
    // Pre-existing behavior, pinned here because the view gate sits directly in front
