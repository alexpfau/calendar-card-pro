...
- 🏷️ **All-Day Events, as a Badge**: An all-day event can say so in a small badge in its calendar's own color instead of in the words _All day_ — [five treatments](https://calendar-card-pro.alexpfau.com/features/event-content#the-all-day-badge) from a bare outline to a solid chip, every color derived from the accent you already set. Off by default; `allday_badge: subtle` is the one to try first
