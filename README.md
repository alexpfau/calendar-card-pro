...
- 🏷️ **All-Day Events, as a Pill**: An all-day event can say so with a rounded pill in its calendar's own color instead of with the words _All day_. [`allday_badge`](https://calendar-card-pro.alexpfau.com/features/event-content#the-all-day-badge) puts it beside the clock or wraps it around the event title the way Google Calendar and Apple Calendar do, and `allday_badge_style` picks one of five treatments from a bare outline to a solid chip — every color derived from the accent you already set. Off by default; `allday_badge: title` is the one to try first
