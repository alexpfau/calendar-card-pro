...
`rendering/styles.ts` and `rendering/editor/styles.ts` and takes 31,579 raw / 12,234 gzip
bytes off the eager path (re-measured at v4.1.0 by building with the plugin and again
without it; the card's own stylesheet is 60% comment by then, and 30,719 of that raw saving
